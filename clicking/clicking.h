...
本作品*采用知识共享署名-非商业性使用-相同方式共享 4.0 国际许可协议进行许可。
...
你可以进行改编，演绎，或者作为交流用途分享给他人，请注明原作者，勿用于商业用途，感谢配合！

*指clicking.h和clicking!.cpp以及产生的的exe和Note.txt，Tutor.txt文件，由lz原创

若有建议或bug，欢迎随时踢馆~: 1612162886@qq.com；洛谷ID：43845(ystkc)
...
//#define CLICK_CUSTOMBUFFER//自定义缓冲区大小（请在include本文件前定义好宏和变量） 
//#define CLICK_CUSTOMWDD//自定义最大窗口个数 （请在include本文件前定义好宏和变量）
//#define CLICK_CUSTOMWDLMT//自定义最大窗口大小（请在include本文件前定义好宏和变量）
...
short int CMessageXLine,CMessageXHeight;/*msgbox() NEEDED*/
pthread_t CRecentTids;
short int CLatestNum,Trans=1;//非焦点窗口可见性  当前运行线程编号
char CLastKey;//全局getch() 
//API SYMBOL
...
//常量 
const int
#ifndef CLICK_CUSTOMWDD //可以在主程序里自己定义，此处仅供参考 
		WDD=5,//窗口数量 数组常量 
#endif
#ifndef CLICK_CUSTOMWDLMT //窗口大小（长边） 数组常量 
		WDLMT=205,
#endif
		wOldColorAttrs=Options[2][2];//默认窗口文字颜色 初始化常量 
int
		CMD_MAXCHAR=512,//PipeCMD最大输出长度 
		EoutDelay=Options[3][1],IDDTFP=Options[3][2],//闪电字幕延时(!>5) 注释终止偏移
		WinVer=GetSystemVersion(),//系统版本（用于修复Win10中文\b的双倍回格） 现在暂时没用了 
		CFontSizeX=Options[2][3],CFontSizeY=Options[2][4];//字体大小 宽*长pixel 初始化常量,要修改请见CCurrentFontSizeX/Y 
//主系统 
short int 
...
namespace clicking
{
	...
	//basic plugins基础插件 
	#ifndef CLICK_SIMPLE
	int INTSc(int linenum, int number)//文件读 
	{
	...
	bool LinePr(char* COptionsFileName, int linenum,string ot)//文件写(把COptionsFileName文件第linenum变为ot)
	{
		if (!strcmp(COptionsFileName,"*A"))
		{
	...
	inline void gotoxy(short int x,short int y)//cursor position setting光标位置设置 
	{
	...
	{
		for(unsigned int i=0;i<CWindowOrder.size();i++)
	...
	void CCreateWindow(int wd,int wdbds,string wdtp,int cwl,int cwt,int cwr,int cwb)//window writing system窗口创建系统
	{
		string tmp="";
	...
	{
		unsigned int cnt=0;
		short int xl=0,tpo=2,CMessageXHeight=0,CMessageXLine=0;//cnt:当前处理到第cnt个字符； 
		string tmp="";
		string l=MessageboxLeftBoard[bdstyle],t=MessageboxTopBoard[bdstyle];//边框字符 
		int maxc=(CConsoleRight-CConsoleLeft)/CFontSizeX-2-sx;//maxc：该框最大的宽度（窗口内） 
		if(maxc<0)return;
		WORD TMPCL=cl;
...
//out of namespace clicking
using namespace clicking;
string HEXR(int k) //dec->hex十进制转十六进制 
{
	string tmp="",ans="";
    while (k)
    {
...
bool VerVerify()//设置检查读取 
{
	...
	{
		TextNormal();
		puts ("The Options(options.txt) have not be initialize or it is modified!");
		puts("程序设置未初始化");
		HightLight();
		cout<<"Please RESTART LATER\n请稍候重启该程序！\n";
		TextDeep();
		cout<<"\nVERSION:"<<"ClickingVersionsV2.8\nCreated by Lz\n"<<"\nReWriting...\n";
		Rst:;	//Reset Mark
		if(!LinePr((char*)"*A",0,""))
		{
			TextDeepRed();
...
{
	unsigned int i=0;
	int p=EoutDelay,lineot=0;//p:调速 (ms)
...
//explain:from(sx,sy)to(ex,ey)将sx,sy到ex,ey之间的空间还原（为原来的窗口内容） 
{
	...
	for (unsigned int ii=0;ii<CWindowOrder.size();ii++)//否则查找当前位置属于哪个窗口 
...
//cwcn来自主线程的传递 
{
	...
	while(CLatestNum==ThreadNum)
	{
		...
		do
		{
			...
			if (CLatestNum!=ThreadNum){OptionsDeep();CEnabled=0;lr();return NULL;}
		}
		while(CPause);
		...
		if(KEY_DOWN(MOUSE_MOVED) && CClickable)//CClickable=0时为预览模式 
		{
			...
			/*if(CValue_sys[bs][(pt.y)/CFontSizeY][(pt.x)/CFontSizeX]==-3)
			{
				...
				if(CWindowOrder[i]-'0'==bs){CWindowOrder.erase(i,1);break;}//cout<<CWindowOrder;char CLastKey=getch();
				OptionsDeep();lr();
