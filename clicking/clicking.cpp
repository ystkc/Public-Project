...
const int WDD=5;//窗口数量 
#endif
#ifndef WDLMT
const int WDLMT=205;//窗口大小（长边） 
#endif
...
public:
	...
	inline bool LinePr(char* COptionsFileName, int linenum,string ot)//文件写(把COptionsFileName文件第linenum变为ot)
	{
	...
	//		if(chc==0)msg.insert(i,1,'1'),i++;
		...
		ClickImplementation::CMessageXHeight=CMessageXHeight+5;ClickImplementation::CMessageXLine=CMessageXLine+5;//备份到全局变量，方便后期请出信息框
		cnt=xl=0;
	...
	//此处不能用/*CTasks_sys[++CTasks_sys[0]]=3;*/，而应该用CShowWindow()，为了插入窗口顺序 
	 ...
	 CShowWindow(3);
	 CShowWindow(1);
	 COpenThread(15,1,1);//COpenThread之前的这些函数都是可以自由调用的，不受CEnabled限制 
...
private:
	...
	//basic plugins基础插件 
#ifndef CLICK_SIMPLE
	inline string HEXR(int k) //dec->hex十进制转十六进制 
	{
		string tmp="",ans="";
	    while (k)
	    {
	...
	void lr()//fast cls光速清屏,调用它相当于system("cls")
	...
	//			cout<<CChar_sys[CCurrentWD][CCurrentX][CCurrentY-1];
			...
			if(KEY_DOWN(MOUSE_MOVED) && CClickable)//CClickable=0时为预览模式 
			{
				...
				//这里使用oldversion好像不对劲，因为他只是还没有启用而已 
